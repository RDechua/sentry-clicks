...
## 2026-06-15: Enforcement pipeline — routing, full audit, review report (Tasks 6.2/6.4/6.5)

`sentry enforce` is the inference/enforcement pipeline: load the trained model + calibrator, score a split, route via the three-tier policy (`route_batch`), log every enforcement ACTION with full SHAP top-5, and render the human-review HTML queue. On a 100k val sample: block 500 / review 2,000 / QA 519 / allow 96,981 → 3,019 audit entries, 500-case review report, in ~10s.

**Audit every enforcement action with SHAP; audit allows via the QA sample, not individually.** §3.9 says every decision produces a log entry, but logging all ~97k allows (let alone 3.7M at full scale) with per-row SHAP is infeasible and isn't what real systems do. The resolution is already built into the design: QA_SAMPLE is precisely the mechanism for auditing the allow population — a random slice of would-be-allows is pulled for review and logged. So every block/review/QA gets a full audit entry with SHAP top-5, and the allow population is audited through its QA sample. This bounds SHAP compute to the routed subset (3k, not 100k — the difference between 10s and 5+ minutes) and matches the QA-sampling rationale from Task 6.1.

**Two scoring conventions, on purpose.** The audit logs BOTH `raw_score` (booster P(legit)) and `calibrated_score` (post-isotonic) — the two fields the Task 1.9 schema reserved. But the DEMO routes on the RAW fraud score, not the calibrated one, for a concrete reason discovered here: isotonic calibration ties a large mass of cases at calibrated fraud = 1.0 (its flat segments collapse many low-p scores), so calibrated-score quantiles degenerate and no review band can form (a first attempt gave block=review=1.0, 0 review cases). The raw score has continuous spread, so quantile thresholds populate all three tiers. Production would route on calibrated scores per the cost model — but the cost model auto-decides everything anyway (no review tier), so a populated review queue is inherently a demonstration construct.

**Demo thresholds are quantile-set, and labeled as such.** Top 0.5% of raw fraud scores → block, next 2% → review. §3.6 forbids percentile thresholds for SELECTION; this is artifact POPULATION, not selection — the operating thresholds are the cost-based ones from Week 5. Without this, the review-queue artifact is empty because the is_attributed-as-fraud proxy makes "block" target ~all of val (the same effect the cost analysis surfaced). The enforce command's docstring and constants say all of this in place.

**The end-to-end shape.** Training and enforcement are separate pipelines — `sentry train` (ingest → features → fit → calibrate, the hours-long artifact build) and `sentry enforce` (load → score → route → audit → report, seconds). That separation is deliberate and defensible: you don't retrain to score. Together they are the "pipeline final form"; the build guide's single-`sentry pipeline` framing is satisfied by the two composable commands plus the still-working tracer (`pipeline --sample`).

**HTML report (6.4).** `triage/report.py` renders the HUMAN_REVIEW queue via an autoescaped Jinja2 template — case id, calibrated fraud score, click metadata, and top-5 SHAP bars (green = pushed toward legit, red = toward fraud, labeled as log-odds so no one reads a bar as a probability). autoescape is the one security-relevant setting (a crafted case_id can't inject markup); a test asserts it. The committed `reports/sample_review_queue.html` is a real 500-case run.

**Confidence:** High — the pipeline runs end to end and produces inspectable artifacts; routing/report/SHAP are unit-tested. **Revisit:** at full-stream scale, enforce would batch over the stream and write audit in chunks; the 100k-sample demo stands in for that.

---

## 2026-06-15: Week 6 retrospective (Task 6.6)

**What got built.** The enforcement layer that turns scores into actions: the three-tier router (`route_case`/`route_batch`), SHAP explanations (`predict.py`, top-5 in log-odds), the full audit writer wired end to end, the human-review HTML report, and the `sentry enforce` pipeline that composes them. About eight commits, ~40 new tests, and a working end-to-end run producing a real audit log and a 500-case review queue.

**What surprised me.** Two things, both downstream of the same root. First, when I ran enforcement on val, the model blocked essentially everything — because `is_attributed=0` ("didn't install") is 99.78% of traffic, and that's what "fraud" maps to. I knew this from the cost analysis intellectually, but seeing the router auto-block 99,999 of 100,000 clicks made it visceral: the attribution label is a poor fraud proxy, and a real system would need a real fraud label, not non-conversion. Second, my first attempt at a demo review queue came back empty because isotonic calibration ties a huge mass of cases at calibrated fraud = 1.0 — the same flat-segment behavior I'd noted as a footnote in Week 4 calibration came back as a concrete blocker here. I had to route the demo on the raw score to get spread. Both are honest findings I'd rather have hit now than in an interview.

**What went smoothly.** The pieces from earlier weeks slotted together with almost no friction — the audit schema from Week 1 took the full entries unchanged, the calibrator and model artifacts loaded cleanly, and the router reused the cost layer's fraud-score direction so there was no second inversion to get wrong. That's the payoff of having built the audit schema in Week 1 before the model existed: when the real entries arrived, the shape was already right.

**The judgment call I'm most ready to defend.** Auditing every enforcement action with SHAP but auditing the allow population through the QA sample rather than logging all 97k+ allows. It's the realistic design (you can't SHAP-log every allowed click at stream scale), it's what QA sampling is *for*, and it keeps the §3.9 "every decision is accountable" guarantee — allows are accountable via the sample, actions individually. It also happens to be what made the run finish in 10 seconds instead of 5 minutes.

**On track?** Yes. The enforcement workflow is real and the end-to-end produces inspectable artifacts. The honest caveat I'm carrying into Week 7's docs: the enforcement *demo* uses quantile thresholds to populate the tiers, because the cost-optimal policy (Week 5) reviews nobody and the label proxy makes blocking target most of the traffic — so the review queue is a machinery demonstration, not a recommended operating point, and policy.md / the README need to say that plainly.

---

## 2026-06-17: Why LightGBM as the model class (Task 7.5 — decision-log audit backfill)

**Why this entry exists.** The Week-7 decisions audit (7.5) checked every "must internalize" item in build-guide §1.4 against the log. Nine of ten were covered; one was not, as a *standalone* rationale: "why LightGBM, not XGBoost or a neural net." The Task 4.3 entry defends the hyperparameters, the class weights, the metric, and the feature-exclusion call — but it assumes the model class rather than arguing it. CLAUDE.md §3.5 names decisions.md as the home for that argument, so here it is. The choice itself was locked on Day 1 (CLAUDE.md §5.3); this backfills the reasoning rather than pretending the decision happened today.

**Why gradient-boosted trees at all (vs. linear or a neural net).** The data is tabular, heterogeneous (categorical identifiers + numeric velocity/rate features), with non-linear interactions and a 0.2% positive class. Gradient-boosted decision trees are the state of the art for exactly this shape: they capture feature interactions without manual crossing, handle mixed types, and are robust to monotone feature scaling. A linear model can't express "high velocity AND low conversion rate" without hand-built interactions (the tracer's logistic regression is the baseline that shows the gap — PR-AUC 0.0102 vs. the tree's 0.5+). A neural net needs far more data per positive, careful categorical embedding, and more tuning to *match* GBDT on tabular data, let alone beat it — and it would forfeit the cheap, exact tree-SHAP the audit log depends on (§3.9). For a few-hundred-thousand-positive tabular problem with an explainability requirement, GBDT is the obvious class.

**Why LightGBM specifically, over XGBoost and CatBoost.** Three properties decided it, all of which map to real constraints in this project:
- **Memory and speed at scale.** LightGBM's histogram binning + leaf-wise growth train faster and in less RAM than XGBoost's level-wise default on large data. With 184.9M source rows in a 3.8 GB container (≈50× more data than memory), this was not a nicety — it's what let training and the density gate run at all. The same constraint that forced the pass-split feature materializer made the model's memory profile a first-order selection criterion.
- **Native NULL handling.** §3.4's "no prior data → NULL" convention (preferred over a sentinel, because trees handle missingness natively) is consumed directly by LightGBM, which learns a default direction for NULLs at each split. XGBoost also handles missing values; the point is that LightGBM let me keep NULL as the honest "no history yet" value end-to-end without an encoding step.
- **Built-in `average_precision` eval metric.** LightGBM 4.6 exposes PR-AUC as a native early-stopping metric, so the round count is chosen by the same metric that is the project headline (§3.2). One less place to hand-roll and get wrong.

**Why not CatBoost in particular.** CatBoost's signature advantages are ordered boosting (target-leakage-resistant categorical encoding) and strong automatic handling of many raw categorical columns. But this project deliberately does *not* feed raw high-cardinality categoricals to the model — raw IP is excluded (F1 entry) and the two interaction strings are excluded (Task 4.3 entry) precisely because tree models memorize them. With the categoricals already reduced to behavioral aggregates, CatBoost's main edge is moot, and it is generally slower and heavier than LightGBM — the wrong trade under the memory constraint.

**The honest caveat.** I'm not claiming LightGBM is uniquely accurate here. On these features at this scale, XGBoost would very likely land in the same PR-AUC ballpark; this is a tabular GBDT problem and the leading libraries converge. The decision is driven by *fit to constraints* — memory/speed in a small container, native NULL semantics, the PR-AUC eval metric, and verified determinism (`deterministic=True` + `force_row_wise=True`, Task 4.3) — not by a claim of accuracy supremacy. That's the defensible interview answer: I chose the library whose engineering profile matched my constraints, on a model class that's correct for the data.

**Confidence:** High. The model class is unambiguously right for tabular imbalanced data with an explainability requirement; the LightGBM-vs-XGBoost call is constraint-driven and I'd say so plainly rather than overclaim. **Revisit:** only if the feature set ever grows to include many raw categoricals (then CatBoost's ordered encoding is worth a bake-off) or if the project moved to a data regime where a different library's profile fit better.

---

## 2026-06-17: Reproducibility verified (Task 7.8)

**Verified reproducibility on 2026-06-17.** Ran the documented README quickstart against the live container (`docker compose run --rm sentry sentry pipeline --sample`). It completed end-to-end in 29.7s: ingested 100,000 sample rows, created the train/val/test split views (60,336 / 20,087 / 19,577), computed all 28 F1+F2+F3 features on train and val, trained the baseline logistic regression on the 26 numeric features, evaluated on val (PR-AUC 0.0175, ROC-AUC 0.8589, Brier 0.4934), and wrote 20,087 audit entries. Exit 0, no errors.

**What this checks and what it doesn't.** It confirms the quickstart works as written and every layer stays wired — ingest → splits → features → model → eval → triage → audit. The quickstart deliberately runs the *baseline* tracer on the tiny 100k sample, so its PR-AUC (~0.018) is expected to be low and is NOT the headline number; the README now says so explicitly to prevent a reader conflating it with the full-model test PR-AUC of 0.559 (which comes from `sentry train`/`enforce` on the full v0.5.0 store). The run is deterministic (seeded logreg, `random_state=42`), so repeated runs reproduce the same metrics. `make check` passes clean (ruff, black, mypy on 61 files, 165 tests, 98% coverage) as the standing reproducibility gate.

**Not re-verified here:** a from-scratch `git clone` + `docker compose build` on a clean machine (the image was already built). The Dockerfile and lockfile are committed, so a fresh build is expected to work, but I'm flagging that the literal "fresh clone on a new machine" step is the one part of 7.8 not re-exercised this session.

---

## 2026-06-17: Week 7 retrospective (Task 7.9)

**What got built.** No new system code — this was the documentation and polish week, and resisting the urge to add features was part of the work. Written or finished: `architecture.md` (stub → full 2,368-word system doc), `policy.md` (12 gray areas drawn from this data, not a template), `adversary-model.md` (three tiers each tied to the actual F1–F4 features, with the Tier-3 gap named), the PRD reconciliation + a plan-vs-reality changelog (Task 7.4), a decision-log backfill for the one missing "must internalize" item (why LightGBM as a class), the README rewritten as a portfolio piece, a code-quality pass (10 missing docstrings added, a stale CLI docstring fixed, `make check` clean at 98% coverage), and a reproducibility check against the live quickstart.

**What surprised me.** Three things, all of the same flavor: writing the docs forced me to *look*, and looking found small lies. (1) `config.py` was an empty stub the whole time — the planned pydantic-settings layer was never needed, because CLI flags plus the `DATA_DIR` mount covered it. That's a clean YAGNI win, but I'd been mentally carrying it as "built." (2) The PRD's architecture diagram pointed audit logging at `src/monitoring/`, a directory that never existed (it's `src/sentry/audit/`). (3) Running the documented quickstart, I watched it print PR-AUC 0.0175 — correct for the baseline tracer on a tiny sample, but a reader would put that next to the README's 0.559 and be confused. None of these were bugs; they were *documentation drift*, and the only way to catch them was to write the doc and then check it against the code.

**What went smoothly.** The decision log paid off enormously. Every doc I wrote this week was assembling material that already existed in `decisions.md` — the cost finding, the §3.4 amendment, the tuning negative result, the F4 descope, the label-proxy gotcha. I wasn't reconstructing reasoning from memory; I was quoting myself. That's the whole point of having logged decisions as I made them: Week 7 is where the deposit gets withdrawn.

**The judgment call I'm most ready to defend.** Keeping the PRD's original "as-planned" prose and adding a changelog, rather than rewriting the PRD to look like it always predicted reality. The honest artifact is "here's what I planned, here's what shipped, here's why they differ" — a recruiter reading the changelog sees judgment, not a suspiciously perfect plan. Rewriting history would have been easier and worse.

**What the documentation reveals that the code doesn't.** The code shows a competent fraud detector. The docs show the *judgment around it* — that I know the `is_attributed` label is a poor fraud proxy, that human review is uneconomic at face-value costs and why, that the system is structurally blind to coordinated Tier-3 fraud, that I'd attack my own conversion-rate features by buying installs. None of that is in the code; all of it is what a T&S team actually hires for. The code answers "can you build it?"; the docs answer "do you understand what you built and where it fails?" — and the second question is the one this project exists to answer.

**If a recruiter looks at this for one minute.** They land on the README and see: a one-line pitch with real numbers (ROC-AUC 0.972 / PR-AUC 0.559), a highlights block that signals policy + adversary + cost + audit (not just "I trained a model"), a working quickstart, a document index, and an honest AI-disclosure. The signal in sixty seconds is "this person did T&S engineering, not a Kaggle notebook" — which is exactly the §9 success definition. The risk in sixty seconds is that the headline number (0.559 PR-AUC) looks low to someone who doesn't register it's a 0.2%-positive problem; the README mitigates that by showing it against the 0.0025 base rate, but it's the one thing a careless skim could misread.

**On track?** Yes — Weeks 1–7 are complete. The code is done and clean; the documentation tells the coherent T&S story §9 asks for. What remains is Week 8 (blog post, LinkedIn, resume, final polish) and the voice-edit pass: every doc drafted by Claude Code this project — tradeoffs.md, the retros, these decision entries, and this week's policy/adversary/architecture/README drafts — needs my own pass to make sure the voice is mine before anything goes public.

---

## 2026-06-17: Final retrospective (Task 8.7)

**What this project taught me about T&S.** That the engineering is the smaller half. I came in thinking fraud detection was a modeling problem; I'm leaving convinced it's a judgment problem wearing a model. The three moments that stuck weren't model improvements — they were a leakage rule I had to *loosen* because my caution had manufactured a 24-hour blind spot, a tuning run that made the model worse (and which I shipped against, documented, rather than fake a win), and a cost analysis that told me the human-review tier I'd assumed was obviously good wasn't worth its price. Each one taught the same thing: in T&S the defensible answer is often the counterintuitive one, and you only find it by refusing to accept a number you don't understand and by putting real costs on the table. The other half of the lesson is that policy *is* the work — writing the gray-areas section forced more genuine thinking than training the model did, because that's where you decide what you're actually willing to act on.

**What I'd do differently if I started over.** Three things, in order of impact. First, I'd fight for a real fraud label instead of building on the attribution proxy. `is_attributed` is a loose stand-in — most non-converting clicks are bored real users, not fraud — and everything downstream inherits that looseness. I documented it honestly everywhere, but the cleaner project starts from confirmed-fraud signal. Second, I'd build the graph/cluster features from the start rather than descoping them; they're the only real counter to the sophisticated, distributed adversary my per-entity features are structurally blind to. Third, I'd tune on full data from the beginning — the subsample shortcut is precisely why my tuning failed, and the negative result, while instructive, cost time I could have spent on the graph layer.

**Honest assessment: where I am vs. where I want to be.** I can now build a defensible end-to-end detection-and-enforcement system and, more importantly, explain why every piece is the way it is — the splits, the metric, the calibration, the cost model, the thresholds, the audit trail. That's a real step up from "I can train a classifier." Where I'm not yet: I haven't worked at production scale with live labels, an adversary actually adapting against me, or a real reviewer team whose capacity and trust I have to design around. I've reasoned about all three in the docs, but reasoning about them and operating them are different, and I won't pretend otherwise in an interview. The gap I most want to close is the feedback loop — designing detection that stays honest when its own blocking decisions censor the next model's training data. That's the hard, real T&S problem and I've only touched its edge.

**The next project.** The graph/cluster layer, as a standalone follow-on: build the bipartite IP↔device / IP↔channel graph, compute connected-component and shared-neighbor features with a per-feature leakage proof, and measure their marginal lift on the Tier-3-shaped errors Sentry currently misses. It's the natural sequel — it targets the exact adversary I named as a gap, it's a genuinely different technical problem (graphs, not tabular trees), and it would let me practice the thing I most need: operating against a moving adversary rather than a fixed dataset. If there's room, I'd pair it with a small drift-and-feedback simulation, because that's where the interesting T&S questions live.

**Closing.** Eight weeks, ~90 commits, seven documents, one model I can defend line by line. The thing I'm proudest of isn't the 0.972 ROC-AUC — it's that the repo reads like someone who understands what they built and where it breaks. That was the actual goal.
