...
**Memory at scale.** `_load_features` reads only the model columns via DuckDB — numerics cast to float32, the two interactions as `category` (LightGBM's native categorical split, no one-hot), label as int8, with a DuckDB memory cap so it doesn't starve LightGBM in the 3.8 GB container. This is the same lean-load discipline the baselines needed.
